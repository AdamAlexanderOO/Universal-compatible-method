...
export default function App() {
  ...
  // Simulated Power Management State & Battery Subsystem
  const [powerState, setPowerState] = useState<PowerManagementState>({
    batteryLevel: 94.4,
    chargeState: 'OPTIMAL',
    batteryHealth: 'PRISTINE',
    healthPercentage: 98.8,
    voltageVolts: 48.24,
    currentAmperes: -1.4,
    powerWattage: 142.6,
    cellTemperatureC: 31.8,
    energyReserveMJ: 47.2,
    maxCapacityMJ: 50.0,
    powerMode: 'BALANCED',
    estimatedRuntimeMin: 340,
    solarFluxEfficiencyPct: 94.8,
  });
  ...
  // Power Management Simulation Loop
  useEffect(() => {
      ...
      setPowerState((prev) => {
        if (!powerOn) {
          ...
          return {
            ...
            currentAmperes: 0,
            powerWattage: 0,
            cellTemperatureC: Math.max(20, prev.cellTemperatureC - 0.2),
          };
        }

        let newLevel = prev.batteryLevel;
        let newTemp = prev.cellTemperatureC;
        let newReserve = prev.energyReserveMJ;
        let newChargeState = prev.chargeState;
        let newAmperes = prev.currentAmperes;
        let newWattage = prev.powerWattage;
        let health = prev.batteryHealth;

        if (prev.powerMode === 'SOLAR_REGEN') {
          // Solar charging
          newLevel = Math.min(100, prev.batteryLevel + 0.35);
          newChargeState = newLevel >= 99.5 ? 'OPTIMAL' : 'REGENERATING';
          newAmperes = +3.8;
          newWattage = 188.0;
          newTemp = Math.max(26, prev.cellTemperatureC - 0.1);
        } else if (prev.powerMode === 'ECO_SAVER') {
          // Eco discharge
          newLevel = Math.max(0, prev.batteryLevel - 0.03);
          newChargeState = newLevel < 15 ? 'DRAINED' : 'OPTIMAL';
          newAmperes = -0.8;
          newWattage = 65.0;
          newTemp = Math.max(24, prev.cellTemperatureC - 0.15);
        } else if (prev.powerMode === 'HYPER_OVERDRIVE') {
          // Overdrive discharge
          newLevel = Math.max(0, prev.batteryLevel - 0.22);
          newChargeState = 'OVERDRIVE';
          newAmperes = -5.4;
          newWattage = 295.0;
          newTemp = Math.min(58, prev.cellTemperatureC + 0.3);
        } else {
          // Balanced mode
          newLevel = Math.max(0, prev.batteryLevel - 0.08);
          newChargeState = newLevel < 15 ? 'DRAINED' : 'DISCHARGING';
          newAmperes = -1.4;
          newWattage = 142.0;
          newTemp = Math.min(42, Math.max(28, prev.cellTemperatureC + (Math.random() * 0.2 - 0.1)));
        }

        newReserve = (newLevel / 100) * prev.maxCapacityMJ;
        ...
        const estMinutes = newAmperes < 0 ? Math.round((newLevel / Math.abs(newAmperes)) * 12) : 999;
        const voltage = 44.0 + (newLevel / 100) * 8.4;
        ...
        return {
          ...
          energyReserveMJ: newReserve,
          currentAmperes: newAmperes,
          powerWattage: newWattage,
          cellTemperatureC: newTemp,
          voltageVolts: voltage,
          estimatedRuntimeMin: estMinutes,
        };
      });
    }, 3500);
  ...
  // Periodic Telemetry Anomaly Broadcasts
  useEffect(() => {
    const alertInterval = setInterval(() => {
      ...
      if (roll < 0.25) {
        const anomalies = [
          {
            title: 'SPECTRAL FLUX SURGE',
            description: 'Transient photon pulse detected on Harmonic Channel #04.',
          ...
          {
            title: 'GEAR ESCAPEMENT JITTER',
            description: 'Micro-torsion deviation recorded at 620 THz coupling.',
            severity: 'WARN' as const,
            category: 'SUBSYSTEM' as const,
            coordinates: '12°S 77°W',
            actionLabel: 'GEAR MATRIX',
            actionPayload: 'GEARS',
          },
          {
            title: 'AI SYNAPSE COHERENCE LOCK',
            description: 'Gemini neural tensor weights achieved 99.8% resonance.',
            severity: 'SUCCESS' as const,
            category: 'AI_CORE' as const,
            actionLabel: 'OPEN AI CORE',
            actionPayload: 'CONSOLE',
          },
        ];
        const selected = anomalies[Math.floor(Math.random() * anomalies.length)];
        dispatchAlert(selected);
      }
    }, 28000);
  ...
  // Landscape & Viewport Orientation Tracker
  ...
  const MOBILE_TABS: { id: MobileTab; label: string; icon: React.ComponentType<{ className?: string }>; tag: string }[] = [
    ...
    { id: 'RADAR', label: 'Cosmic Radar', icon: Radio, tag: '04' },
  ...
  const handleRunSimulation = async (mode: SimulationMode) => {
    ...
    try {
      ...
      dispatchAlert({
        title: `SIMULATION COMPLETE: ${mode}`,
        description: data.output.title || 'AI Synapse matrix converged on optimal photon state.',
        severity: 'SUCCESS',
        category: 'SIMULATION',
        source: 'AI_CORE',
      ...
      dispatchAlert({
        title: `SIMULATION COMPLETE: ${mode}`,
        description: fallbackResult.output.title,
        severity: 'SUCCESS',
        category: 'SIMULATION',
        source: 'LOCAL_AI_SIM',
      });
  ...
  const handleOverchargeCore = () => {
    ...
    dispatchAlert({
      title: 'CORE OVERCHARGE PULSE',
      description: 'Maximum photon surge broadcasted across all 8 subsystem matrix channels.',
      severity: 'WARN',
      category: 'POWER',
      source: 'CORE_REACTOR',
    });
  ...
  return (
    <div
      ...
      className="relative min-h-screen w-full text-white font-sans flex flex-col pb-8 overflow-x-hidden transition-colors duration-300"
      style={{
    ...
    >
      ...
      {!isArcadeFullscreen && (
        <header
          className="sticky top-0 z-40 border-b px-3 sm:px-4 py-2 sm:py-2.5 select-none backdrop-blur-md transition-colors duration-300"
          style={{
            borderColor: 'rgba(255, 255, 255, 0.12)',
            backgroundColor: `${currentTheme.bgPanel}F4`,
          }}
        >
          <div className="max-w-6xl mx-auto flex items-center justify-between gap-2">
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <div className="text-[9px] font-mono text-neutral-400 mt-0.5 flex items-center gap-1.5">
                  <span>{fluxFrequency.toFixed(1)} GHz</span>
                  <span>•</span>
            ...
            <div className="flex items-center gap-1 sm:gap-1.5 shrink-0">
              {/* Visual Battery Level Indicator & Power Matrix Subsystem */}
              <BatteryStatusIndicator
                powerState={powerState}
                onUpdatePowerState={setPowerState}
                theme={currentTheme}
              />
              ...
              <button
                ...
                className="flex items-center gap-1.5 px-2 py-1 rounded bg-white/5 border border-white/15 hover:bg-white/10 active:scale-95 transition-all text-xs font-mono min-h-[34px]"
              ...
              <button
                ...
                className={`p-1.5 sm:px-2 sm:py-1 rounded border text-xs font-mono flex items-center gap-1 active:scale-95 transition-all min-h-[34px] ${
                  isDiagnosticsOpen
              ...
              <button
                ...
                className="flex items-center gap-1 px-2 py-1 rounded bg-white/5 border border-white/15 hover:bg-white/10 active:scale-95 transition-all text-xs font-mono min-h-[34px]"
              ...
              <button
                ...
                className="p-1.5 sm:p-2 rounded bg-white/5 border border-white/15 text-neutral-300 hover:text-white active:scale-95 min-h-[34px] min-w-[34px] flex items-center justify-center"
                title="Sound"
              >
      ...
      {!isArcadeFullscreen && (
        <div
          className="border-b px-2.5 sm:px-4 py-2 overflow-x-auto select-none scrollbar-none transition-colors duration-300"
          style={{
            ...
            {MOBILE_TABS.map((tab) => {
              ...
              return (
                <button
                  ...
                  className={`px-3 py-1.5 rounded text-[11px] font-mono font-bold tracking-wider whitespace-nowrap flex items-center gap-1.5 transition-all border shrink-0 min-h-[36px] ${
                    isActive
                      ? 'text-white'
      ...
      {!isArcadeFullscreen && (
        <div className="bg-[#060606] border-b border-white/10 font-mono text-center select-none py-1.5 px-2.5">
          <div className="max-w-6xl mx-auto grid grid-cols-4 gap-1.5">
            ...
            <div className="p-1 border border-white/5 bg-white/[0.02] rounded">
              <div className="text-[8px] text-neutral-400 uppercase">Cosmic Radar</div>
              <div className="text-xs font-bold text-amber-400">10^12 pc Range</div>
            </div>
          </div>
        </div>
      )}
      ...
      <main className={`flex-1 ${isArcadeFullscreen ? 'p-0 max-w-none w-full' : 'p-2.5 sm:p-4 max-w-6xl mx-auto w-full pb-10'}`}>
        ...
        {/* TAB 4: TACTICAL & TRILLION-PARSEC COSMIC RADAR HUD */}
        {activeTab === 'RADAR' && (
          <div className="space-y-3">
            <div className="flex items-center justify-between pb-1.5 border-b border-white/10 font-mono text-xs">
              <span className="font-bold animate-pulse" style={{ color: currentTheme.primary }}>
                04 // TRILLION-PARSEC MACRO-QUANTUM RADAR & TACTICAL HUD
              </span>
              <span className="text-neutral-400 font-mono">10¹⁴ pc HORIZON</span>
            </div>
            <div className="border border-white/10 bg-neutral-950/80 p-2 sm:p-4 rounded-lg">
      ...
      {/* Real-Time Slide-in Alert Ticker Notifications */}
      <AlertTicker
        theme={currentTheme}
        onNavigateTab={(tab) => {
